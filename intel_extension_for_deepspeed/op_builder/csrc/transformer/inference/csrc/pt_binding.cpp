...
#include "inference_onemkl_wrappers.h"
...
at::Tensor einsum_sec_sm_ecm(at::Tensor& Q, at::Tensor& W)
{
    ...
    */
    ...
    unsigned k = Q.size(0);
    onemkl_gemm_ex(InferenceContext::Instance().GetSyclHandle(),
                       ...
                       int window_size)
{
    ...
    *(InferenceContext::Instance().GetSyclHandle()) =
        *(InferenceContext::Instance().GetCurrentStream());
    onemkl_strided_batched_gemm(InferenceContext::Instance().GetSyclHandle(),
                                soft_len,
                                seq_len,
                                k,
                                &alpha,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
    m.def("softmax_context_int8",
          ...
          "DeepSpeed attention with int8 (SYCL)");

    // The following functions handle type dispatching internally
    m.def("gated_activation", &ds_gated_activation, "DeepSpeed Bias GEGLU (SYCL)");
    ...
    m.def("layer_norm_residual_store_pre_ln_res",
          ...
          "DeepSpeed layer norm + store pre Layernorm residual (SYCL)");
    m.def("rms_norm", &ds_rms_norm, "DeepSpeed rms norm (SYCL)");
    m.def("pre_rms_norm", &ds_pre_rms_norm, "DeepSpeed pre rms norm (SYCL)");
    m.def("_vector_add", &_vector_add, "DeepSpeed vector add (SYCL)");
    m.def("apply_rotary_pos_emb", &apply_rotary_pos_emb, "DeepSpeed mlp with fp16 (SYCL)");
    m.def("moe_res_matmul", &moe_res_matmul, "DeepSpeed moe residual matmul (SYCL)");
...
#define DEF_OPS(_name, _dtype)                                                                    \
    ...
    m.def("softmax_context_" #_name,                                                              \
          ...
          "DeepSpeed attention with " #_name " (SYCL)");                                          \
    m.def("bias_gelu_" #_name, &ds_bias_gelu<_dtype>, "DeepSpeed Gelu with " #_name " (SYCL)");   \
    m.def("bias_add_" #_name, &ds_bias_add<_dtype>, "DeepSpeed Bias Add with " #_name " (SYCL)"); \
    m.def("bias_relu_" #_name, &ds_bias_relu<_dtype>, "DeepSpeed ReLU with " #_name " (SYCL)");   \
    m.def("bias_residual_" #_name,                                                                \
          &ds_bias_residual<_dtype>,                                                              \
          "DeepSpeed residual-bias add with " #_name " (SYCL)");                                  \
    m.def("qkv_gemm_" #_name, &ds_qkv_gemm<_dtype>, "DeepSpeed qkv gemm with " #_name " (SYCL)"); \
    m.def("rms_qkv_gemm_" #_name,                                                                 \
          &ds_rms_qkv<_dtype>,                                                                    \
          "DeepSpeed rms qkv gemm with " #_name " (SYCL)");                                       \
    m.def("mlp_gemm_" #_name, &ds_mlp_gemm<_dtype>, "DeepSpeed mlp with " #_name " (SYCL)");      \
    m.def("rms_mlp_gemm_" #_name,                                                                 \
          &ds_rms_mlp_gemm<_dtype>,                                                               \
          "DeepSpeed rms mlp gemm with " #_name " (SYCL)");                                       \
    ...
    m.def("linear_layer_" #_name,                                                                 \
          ...
          "DeepSpeed linear_layer with " #_name " (SYCL)");                                       \
    ...
    m.def("dequantize_" #_name,                                                                   \
          ...
          "DeepSpeed dequantize with " #_name " (SYCL)")
