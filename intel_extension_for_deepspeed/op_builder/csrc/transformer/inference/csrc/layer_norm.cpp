...
public:
    fused_ln(T* output, const T* vals, const T* gamma, const T* beta, float epsilon, int elems_per_row) : output(output), vals(vals), gamma(gamma), beta(beta), epsilon(epsilon), elems_per_row(elems_per_row) {};
    ...
    const int block_offset = (tb.get_group_id(1) * (maxThreads / threadsPerGroup) * elems_per_row) +
                             (tb.get_local_id(0) * elems_per_row);
    const int thread_offset = tb.get_group_id(1) * T_per_load;
    const int base_offset = block_offset + thread_offset;
...
#define LAUNCH_FUSED_LN(unRollFactor, threadsPerGroup, maxThreads) \
{   \
    fused_ln<T, unRollFactor, threadsPerGroup, maxThreads> fn(output, vals, gamma, beta, epsilon, elems_per_row);     \
...
template void launch_fused_ln(sycl::half*,
                              ...
                              int,
                              sycl::queue);
template void launch_fused_ln(bf16*,
                              const bf16*,
                              const bf16*,
                              const bf16*,
                              float,
                              ...
                              sycl::queue);
template void
launch_fused_ln(float*, const float*, const float*, const float*, float, int, int, sycl::queue);
...
template void launch_fused_residual_ln(bf16*,
                                       const bf16*,
                                       const bf16*,
                                       const bf16*,
                                       const bf16*,
...
template void launch_fused_residual_ln_store_pre_ln_res(bf16*,
                                                        bf16*,
                                                        const bf16*,
                                                        const bf16*,
                                                        const bf16*,
