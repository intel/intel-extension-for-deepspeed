...
namespace at {
  namespace dpcpp {
    sycl::queue * getCurrentSYCLStream() {
...
public:
    InferenceContext()
        ...
        int stat = DPCT_CHECK_ERROR(_syclHandle = &dpct::get_in_order_queue());
        if (stat != 0) {
            // It would be nice to use syclGetStatusName and
            // syclGetStatusString, but they were only added in CUDA 11.4.2.
            auto message = std::string("Failed to create sycl handle: syclStatus_t was ") +
                           std::to_string(stat);
...
if (!_free_memory_size) {
    ...
    sycl::event * GetCompEvent(int id) { return id == 1 ? _comp1_event : _comp2_event; }
    ...
    {
        if (!_comm_stream)
            _comm_stream = async_op ? at::dpcpp::getStreamFromPool(true)
    ...
    sycl::queue * GetCurrentStream(bool other_stream = false)
    {
        ...
        if (other_stream) {
            if (!_stream) _stream = at::dpcpp::getStreamFromPool(true);
            return _stream;
        }
        sycl::queue * stream = at::dpcpp::getCurrentSYCLStream();
        return stream;
    }
    ...
    sycl::queue * GetSyclHandle() { return _syclHandle; }
