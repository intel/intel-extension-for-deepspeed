...
struct fmha_bwd_block_t {
  ...
  using rowsum_t = xetla_vector<acc_T, tile_shape_brxd::sg_tile_size_y>;
  ...
  struct arguments_t {
    ...
    input_T* ptr_dP = nullptr; // TODO: delete
    input_T* ptr_p = nullptr; // debug
    input_T* ptr_dS = nullptr;
    ...
    void debuger_config(input_T* ptr_dropmask_ = nullptr) {
  ...
  __XETLA_API KERNEL_FUNC void operator()(
      ...
      arguments_t& args,
      int max_outer_loop_steps /*, uint32_t slm_base = 0*/) {
    ...
    mem_desc_l_m_t mem_desc_l_m, mem_desc_D;

    mem_desc_brxbc_trans_t mem_desc_trans_p;

    mem_desc_c_t mem_desc_c;
    mem_desc_debug_t mem_desc_debug;

    tile_Tr_t mat_dO;
    tile_Tr_t mat_O;
    tile_Tr_t mat_dQ;

    tile_l_m_t tile_L, tile_D;

    tile_payload_Tr_t tile_payload_Tr, tile_payload_Tr_1, tile_payload_Tr_2;
    tile_payload_l_m_t tile_payload_L;
    tile_payload_l_m_t tile_payload_D;
    ...
    for (int inner_loop_idx = 0; inner_loop_idx < steps; inner_loop_idx++) {
      ...
      mem_desc_D.init(
          args.ptr_D,
          {boundary_m_Tr, 1, boundary_m_Tr},
          {start_m_Tr + brgemm_brxd_t::get_matC_offset_y(g_brxd), 0});
      ...
      xetla_vector<acc_T, tile_shape_brxd::sg_tile_size_y> local_sum =
          tile_reduce<reduce_op::sum, acc_T, acc_T, 1, matAcc_brxd_t>(matAcc_D);
      wg_reduce_sum_t wg_reduce_sum(sg_idx, nbarrier_id, slm_base_addr);
      m_vec_d = wg_reduce_sum(local_sum);
    ...
    for (int outer_loop_idx = 0; outer_loop_idx < max_outer_loop_steps; outer_loop_idx++) {
      ...
      start_k_Tr = outer_loop_idx * gemm_brxd_block_tile_t::blocked_K + ei.get_group(2) * wg_tile_k_Tr;
      ...
      for (int inner_loop_idx = begin; inner_loop_idx < steps; inner_loop_idx++) {
        ...
        { // softmax
          ...
          tile_payload_L.init(mem_desc_l_m);
          tile_load(tile_L, tile_payload_L);

          tile_broadcast_op<tile_minus, matAcc_brxbc_t>(matAcc_p, tile_L.reg);
        ...
        { // D_i= rowsum(dO_i * O_i)
          ...
          // );
          // epilogue_brxbc(g_brxbc, matAcc_dP, mem_desc_debug);
          ...
          mem_desc_D.init(
              ...
              {start_m_Tr + brgemm_brxd_t::get_matC_offset_y(g_brxd), 0});
          tile_payload_D.init(mem_desc_D);
          tile_load(tile_D, tile_payload_D);
          
          tile_broadcast_op<tile_minus, matAcc_brxbc_t>(matAcc_dP, tile_D.reg);
