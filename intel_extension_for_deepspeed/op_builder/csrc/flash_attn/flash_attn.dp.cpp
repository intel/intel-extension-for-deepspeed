...
std::vector<torch::Tensor> flash_attn_fwd(torch::Tensor &q,
    ...
    softmax_L = torch::empty({bs * head_number, 1, seqlens}, q.options().dtype(at::kFloat));
...
std::vector<torch::Tensor> flash_attn_bwd(torch::Tensor &gradout,
    ...
    torch::Tensor dq_acc = torch::zeros(q_.sizes(), q_.options().dtype(at::kFloat));
    torch::Tensor dq = torch::empty_like(q_);
    ...
    void *dq_acc_ptr = (void *)dq_acc.data_ptr();
