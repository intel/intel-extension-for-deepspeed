...
bool flash_scaled_attn_bf16_inf(sycl::queue &queue,
...
static int flash_scaled_attn_bf16_fwd_run(
        sycl::queue &queue, typename P::arguments_t &args) {
    P kernel(args);
...
bool flash_scaled_attn_bf16_fwd(sycl::queue &queue,
    ...
    if (Hs == 128) {
        ...
        P::dtype_m *ptr_m = reinterpret_cast<P::dtype_m *>(
                const_cast<void *>(softmax_workspace));
        P::dtype_b *ptr_b = 
                = reinterpret_cast<P::dtype_b *>(const_cast<void *>(out_buffer));
        arguments_t args(Bs, Hn, Sl, hs_rsqrt_scale, ptr_q, ptr_k, ptr_v, ptr_o,
                ptr_m, ptr_b);
