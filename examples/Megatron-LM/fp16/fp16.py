        ...
        for i, param_group in enumerate(self.optimizer.param_groups):
            ...
            for i, param in enumerate(param_group['params']):
                if param.requires_grad:
                    half_tensor_type_string = 'torch.{}.HalfTensor'.format(get_accelerator().device_name())
                    float_tensor_type_string = 'torch.{}.FloatTensor'.format(get_accelerator().device_name())
                    if param.type() == half_tensor_type_string:
                        self.maybe_print("FP16_Optimizer received {} with {}"
                                         .format(half_tensor_type_string, param.size()))
                    ...
                    elif param.type() == float_tensor_type_string:
                        self.maybe_print("FP16_Optimizer received {} with {}"
                                         .format(float_tensor_type_string, param.size()))
                        fp32_params_this_group.append(param)
                        param_group['params'][i] = param
                    else:
                        raise TypeError("Wrapped parameters must be either "
                                        "{} or {}. "  
                                        "Received {}".format(float_tensor_type_string,
                                                             half_tensor_type_string,
                                                             param.type()))
