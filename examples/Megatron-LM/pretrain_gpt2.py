...
def get_model(args):
    ...
    # GPU allocation.
    model.to(torch.device(get_accelerator().current_device_name()))

    # Fp16 conversion.
    if args.fp16:
        model = FP16_Module(model)
    elif args.bf16:
        model = BF16_Module(model)

    # Wrap model for distributed training.
    if USE_TORCH_DDP:
        i = torch.device(get_accelerator().current_device_name())
...
def get_optimizer(model, args):
    ...
    if args.fp16:
        ...
        optimizer = FP16_Optimizer(optimizer, static_loss_scale=1.0)
...
def see_memory_usage(message, force=False):
    ...
    if dist.get_rank() == 0:
        print(message)
        print("Memory Allocated ", get_accelerator().memory_allocated()/(1024*1024*1024), "GigaBytes")
        print("Max Memory Allocated ", get_accelerator().max_memory_allocated()/(1024*1024*1024), "GigaBytes")
        print("Cache Allocated ", get_accelerator().memory_cached()/(1024*1024*1024), "GigaBytes")
        print("Max cache Allocated ", get_accelerator().max_memory_cached()/(1024*1024*1024), "GigaBytes")
        print(" ")
...
def train(model, optimizer, lr_scheduler,
    ...
    total_time = 0
    total_index = 0
    prof_ite = args.train_iters - 1
    prof_enabled = False
    while iteration < args.train_iters:
        if iteration == prof_ite:
            prof_enabled = True
        else:
            prof_enabled = False
        begin = datetime.now()
        print(f"to run iteration {iteration}", "at", begin.strftime('%Y-%m-%d %H:%M:%S'))
        with torch.autograd.profiler_legacy.profile(enabled=prof_enabled, use_xpu=True) as prof:
            lm_loss, skipped_iter = train_step(train_data_iterator,
                                           ...
                                           lr_scheduler,
                                           args, timers)
            get_accelerator().synchronize()    # for safty to profile cpu/xpu time, should be removed at last
        end = datetime.now()
        diff = end - begin
        print(f"iteration {iteration} is finished", "at", end.strftime('%Y-%m-%d %H:%M:%S'), "(", diff.total_seconds(), "seconds)")

        if iteration > 2 and iteration < prof_ite:
            total_time += diff.total_seconds()
            total_index += 1
            print("average time per iteration (ignore iter 0/1/2):", total_time/total_index, "seconds at iteration ", iteration)

        if prof_enabled:
            print(prof.key_averages().table(sort_by="self_xpu_time_total", row_limit=-1))

        # check the GPU memory after each iteration
        if not args.disable_sysmon:
            sysmon = '/home/adnguye1/repos/tools/pti-gpu/tools/sysmon/build/sysmon'
            if os.path.isfile(sysmon):
...
def initialize_distributed(args):
    ...
    if args.deepspeed:
        ...
        device = args.rank % get_accelerator().device_count()
...
def get_train_val_test_data(args):
    ...
    if mpu.get_model_parallel_rank() == 0:
        ...
        token_counts = get_accelerator().LongTensor([after, eod_token, int(args.do_train), int(args.do_valid), int(args.do_test)])
    else:
        token_counts = get_accelerator().LongTensor([0, 0, 0, 0, 0])

    # Broadcast num tokens.
    if mpu.get_model_parallel_group().size() > 1:
...
if __name__ == "__main__":
    ...
    version_file = sys.path[0] + '/versions.log'
    if os.path.isfile(version_file):
        subprocess.call(['cat', version_file])
    main()
    end = datetime.now()
    diff = end - begin
    print("end process", os.getpid(), "at", end.strftime('%Y-%m-%d %H:%M:%S'), "(", diff.total_seconds()/60.0, "minutes)")
