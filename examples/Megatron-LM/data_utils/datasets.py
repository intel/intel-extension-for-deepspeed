...
from .lazy_loader import lazy_array_loader, exists_lazy, make_lazy, get_lazy_path
...
class json_dataset(data.Dataset):
    ...
    def __init__(self, path, tokenizer=None, preprocess_fn=None, binarize_sent=False,
        ...
        for j in self.load_json_stream(self.path):
            ...
            if self.is_lazy and len(self.X) % json_dataset.CACHE_LEN == 0:
                if self.make_lazy(self.path, self.X):
                    return
                del self.X[:]

        if self.is_lazy:
            self.make_lazy(self.path, self.X, is_pkl=True)
    ...
    def make_lazy(self, path, strs, data_type='data', is_pkl=False):
        """
        ...
        if is_pkl:
            pkl.dump(self.str_lens, open(lenpath, 'wb'))
