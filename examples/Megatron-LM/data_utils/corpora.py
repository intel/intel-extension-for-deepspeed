...
class c4_en(json_dataset):
    """
    dataset for c4/en with arguments configured for convenience

    command line usage: `--train-data c4/en`
    """
    PATH = 'data/c4/en/c4-train.json'
    assert_str = "make sure to set PATH for c4/en data_utils/corpora.py"
    def __init__(self, **kwargs):
        assert os.path.exists(c4_en.PATH), \
                        c4_en.assert_str
        if not kwargs:
            kwargs = {}
        kwargs['text_key'] = 'text'
        kwargs['loose_json'] = True
        super(c4_en, self).__init__(c4_en.PATH, **kwargs)
