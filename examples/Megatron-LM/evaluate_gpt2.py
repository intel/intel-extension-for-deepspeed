...
def evaluate(data_loader, model, args, timers,
    ...
    if num_iterations is not None:
        ...
        if mpu.get_model_parallel_rank() == 0:
            max_iters_gpu = get_accelerator().LongTensor([len(data_loader)])
        else:
            max_iters_gpu = get_accelerator().LongTensor([0])
...
def get_eval_data(args):
    ...
    if mpu.get_model_parallel_rank() == 0:
        ...
        token_counts = get_accelerator().LongTensor([after, eod_token, num_examples,
                                              num_original_tokens,
