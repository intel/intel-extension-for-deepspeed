...
def report_memory(name):
    ...
    string += ' | cached: {}'.format(get_accelerator().memory_cached() / mega_bytes)
    string += ' | max cached: {}'.format(
        get_accelerator().max_memory_cached()/ mega_bytes)
    print_rank_0(string)
...
def load_checkpoint(model, optimizer, lr_scheduler, args):
    ...
    if not release and not args.finetune and not args.no_load_rng:
        try:
            ...
            get_accelerator().set_rng_state(sd['cuda_rng_state'])
