...
def generate_samples(model, tokenizer, args, device):
    ...
    with torch.no_grad():
        while True:
            ...
            terminate_runs_tensor = get_accelerator().LongTensor([terminate_runs])
            ...
            context_length_tensor = get_accelerator().LongTensor([context_length])
...
def main():
    ...
    #generate samples
    generate_samples(model, tokenizer, args, get_accelerator().current_device_name())
